...
$message = ""; // Contains the error/success message 
...
// If user is not an admin, redirect user to the client home
if(!Auth::isAdmin()) {
  header('location: ../client');
  return;
}

// If admin wants to update his/her account settings
...
// Fetch admin info
$adminInfo = ($conn->query("SELECT * FROM $usersTable WHERE id='" . Auth::getLoginCookie() . "'"))->fetch_assoc();

// Get current page
$page = $conn->real_escape_string(empty($_GET['page']) || $_GET['page'] == '/' ? 'home' : $_GET['page']);
...
<head>
  ...
  <link href="vendor/bootstrap/plugins/DataTables/datatables.css" rel="stylesheet" type="text/css">
